...
% Zheng et al. 2016 Fig S1B
%f_ode=@(x, k)[50.86*x(1)^2.0/(x(1)^2. + 0.02315^2.)*17.64^2./(17.64^2. + x(2)^2.) + 0.1 - 0.09367*x(1);
%              17.43*x(1)^2.0/(x(1)^2. + 5.230^2.)*1.038^2./(1.038^2. + x(3)^2.) + 0.1 - 2.699*x(2);
%              69.57*x(3)^2.0/(x(3)^2. + 1.000^2.)*0.02100^2./(0.02100^2. + x(2)^2.) + 0.1 - 0.1503*x(3)];
...
%% Don't modify the code below unless with good reason
%__________________________________________________________________________
%__________________________________________________________________________
%__________________________________________________________________________
%__________________________________________________________________________
...
% here is the symbolic part to define jacobian matrix that was making
% trouble in R2019a matlab
...
for i = 1:length(SteadyState_save(:,1))
    ...
    if System_save(SteadyState_save(i,1),length(k)+1) ~= 1 && SteadyState_save(i,n+2) ~= 2 % Exclude oscillators and unstable solutions
        ...
        %if egenvalues define system as unstable --> delete entry from steady state entries.
        if s == 1 && System_save(SteadyState_save(i,1),k_length+2) ~= 0 
        ...
        %If diffusion should be evaluated (default setting) AND the system converges to a steady state
        if evaluate_diffusion == 1 && y == 1 
            for j = 1:length(binary_diffusor(:,1))
                ...
                % This function will output a diffusion matrix if the node number of diffusing nodes is within the range of m_min and m_max. 
                % In addition a binary output w1 is parsed to the following if loop.
                % The first diffusing node is always set to 1; other nodes diffusion rate is sampled according to the d_range
                %  w = 0, no diffusion has to be evaluted in main script;  w = 1 so that diffusion is evaluted in main script
                [w1,D_matrix] = CreateDiffusionMatrix(n,m_min,m_max,d_range,diffusing_nodes); 
                ...
                if w1 == 1
                    for p1 = 1:length(D_matrix(1,:))
                        ...
                        %Function IsTuring takes in the substituted form of the Jacobian matirx as well as 
                        % a vector defining the diffusion parameters for each node. 
                        % Next eigenvalues are calculated of the Jacobian INCLUDING diffusion. 
                        % w2 defines if and which type of Turing instability (1 or 2) has been found.
                        w2 = IsTuring(S,D,t);
