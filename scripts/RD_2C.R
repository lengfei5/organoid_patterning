...
# Script purpose: test the network topology in RD system
# original codes were from https://gitlab.com/adeconomou/ruga-patterning-quantifications-and-simulations
# Usage example: 
# Author: Jingkui Wang (jingkui.wang@imp.ac.at)
# Date of creation: Mon May 10 16:24:09 2021
##########################################################################
##########################################################################
...
if(RD_2C){
  ...
  source('Functions_RD_2C_simulation.R')
  
  #define topologies for activitor-inhibitor and substrate depletion model
  ...
  for(i in 1:2){
    ...
    print(i)
    cat("Generate Parameters \n")
    
    ##########################################
    # PART 1 : generate parameters of Jacobian matrix
    ##########################################
    Y<-numeric()
    
    print(system.time(Y<- aaply(X, 1, generate_parameters_2c, top=topo[[i]])))
    
    # jacobian matrix, phase, min.wavenumber, max(Re(eig$values)),abs(eigvec)
    colnames(Y) <- c("Type","Da","Aa","Ab","Db","Ba","Bb","ph","k2","l","va","vb")
    
    Y <- Y[!is.na(Y[,1]), ]
    
    parsets<-Y[Y[,1]==1, ] # parameters with instability
    
    
    ##########################################
    # PART 2 SCALE DATASETS
    ##########################################
    cat("parameters scaling \n")
